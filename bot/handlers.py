...
def handle_claim(cmd: ParsedCommand, client) -> bool:
    ...
    try:
        ...
        try:
            send_reply(client, format_error(handle))
        except Exception:
            pass
        return False
...
def handle_reveal(cmd: ParsedCommand, client) -> bool:
    ...
    try:
        send_reply(client, f"@{handle} — scoring in progress, give me a moment ⏳")
...
def handle_inspect(cmd: ParsedCommand, client) -> bool:
    ...
    if not target:
        send_reply(client, format_inspect_missing_target(issuer))
        return True
    ...
    try:
        ...
        try:
            send_reply(client, format_error(issuer))
        except Exception:
            pass
        return False
...
def handle_unknown_command(cmd: ParsedCommand, client) -> bool:
    ...
    try:
        send_reply(client, format_unknown_command(cmd.issuer_handle, cmd.raw_content))
        return True
    except Exception as e:
